...
- 신용카드 거래 내역 파싱
- 카드번호 및 이용카드 정보 기반 owner 자동 감지 (태우/채영)
- 신한카드: 다양한 날짜 구분자(., -, /) 및 시간 포함 형식 대응
- 승인일시, 가맹점명, 금액 추출
...
- 날짜/시간 파싱 및 정규화
- **Transaction Status**: 상태(취소, 승인 등)를 감지하여 `status`에 저장
- **Amount Normalization**: 모든 거래의 금액은 절대값 또는 발생 방향에 따른 양수(+)로 1차 처리
...
- **Positive = Increase 원칙**: 자산/부채의 증가, 비용/수익의 발생은 모두 양수(+)로 변환
- 계정 매핑 및 분류 규칙 적용 (태우/채영 계정 분리)
- 카드 취소 내역(`status == "canceled"`) 제외 처리
- 복식부기 원칙:
  - 왼쪽(Debit): 자산 증가, 비용 발생 (+)
  - 오른쪽(Credit): 자산 감소, 수익 발생 (-) -> 단, Whooing 변환 시 부호에 따라 자동 배치

**`build_whooing_rows(postings, config)`**

- 포스팅을 Whooing 업로드 형식으로 변환 (날짜, 아이템, 금액, 왼쪽, 오른쪽, 메모, 시간)
- **부호 기반 사이드 배치**:
  - 자산 증가(+) -> 왼쪽 / 자산 감소(-) -> 오른쪽
  - 부채 증가(+) -> 오른쪽 / 부채 감소(-) -> 왼쪽
  - 비용 발생(+) -> 왼쪽
...
- 카드: 카드번호 또는 이용카드 명칭 기반
- 토리/온누리: 이름 기반 또는 파일명 파라미터
- 은행: 파일명 또는 명시적 파라미터

**전용 계정 매핑:**

- 태우: `신용카드태우`, `우리은행태우`, `온누리태우`, `토리태우`
- 채영: `신용카드채영`, `우리은행채영`, `온누리채영`, `토리채영`

### 5. 아이템 정제 (Item Cleaning)

**불필요 접두어 제거:**

- 은행 거래 내역의 "펌뱅킹", "체크우리", "타행건별", "오픈뱅킹" 등 금융사 범용 용어를 자동으로 제거하여 실제 가맹점명(아이템)만 추출

**정보 보강:**

- 가맹점명이 모호할 경우(`description`에 상세 정보가 있는 경우) `description`을 아이템으로 우선 사용

### 6. 통합 부호 체계 (Sign Convention)

**"Positive = Increase" 원칙:**

- 시스템 내부(`Posting`)에서는 자산의 증가, 부채의 증가, 비용의 발생, 수익의 발생을 모두 **양수(+)**로 취급합니다.
- `build_whooing_rows` 단계에서 각 계정의 성격(자산/부채/비용/수익)과 부호를 조합하여 최종적으로 Whooing의 왼쪽(차변)/오른쪽(대변)에 배치합니다.
- 이를 통해 은행 출금(자산 감소), 카드 사용(부채 증가), 포인트 적립(자산 증가/수익 발생) 등을 일관된 논리로 처리합니다.
...
### 7. 최근 주요 수정 사항 (2026.01)

- **네이버페이 포인트 적립 누락 해결**: 포인트 가계부의 "적립/사용" 통합 파싱 및 수익 계정 매핑(네이버포인트지급)
- **아이템 정제 로직 강화**: 금융권 범용 루틴 명칭(펌뱅킹 등) 제거 및 상세 설명 우선 적용
- **카드 거래 owner 매칭 정교화**: 현대카드뿐 아니라 신한카드도 카드번호 기반으로 태우/채영 자동 분류 및 전용 계정(`신용카드채영` 등) 매핑
- **부호 체계 단일화**: "Positive = Increase" 원칙 도입으로 데이터 흐름 가독성 및 유지보수성 향상
- **취소 거래 제외**: 사용자 요청에 따라 카드 결제 취소 건을 최종 결과물에서 필터링
...
class StdTxn:
    ...
    status: str            # 상태 (posted, canceled)
    payment_method: str    # 결제 수단 (points, money, topup 등)
    raw_ref: str           # 원본 참조
```
